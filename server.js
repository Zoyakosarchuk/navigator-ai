...
function buildMessages(profile) {
  ...
  // ===== Ветка «Своё дело» (предприниматели) =====
  if (/Своё дело|предприним/i.test(track)) {
    const bsys = [
      'Ты — сборная роль лучших в мире бизнес-стратегов и наставников предпринимателей, специалистов по подбору ниши.',
      'Ты собираешь персональный разбор «Своё дело» для российского сервиса «Навигатор будущего».',
      '',
      'ПРАВИЛА:',
      '1. Пиши по-русски, тёплым, но деловым языком, обращайся на «вы».',
      '2. Принцип асимметрии: живой человеческий ритм, разная длина фраз, без канцелярита и клише.',
      '3. Опирайся только на переданные данные профиля (сильные стороны, ценности, мотивация и блок «бизнес»: тип основателя, формат, отношение к риску и деньгам, ресурсы, идея). Домыслы запрещены.',
      '4. Если в поле бизнес.идея указана идея или сфера — подбирай ниши внутри неё. Если идеи нет — предложи направления под склад человека.',
      '5. В разделе рисков говори честно и прямо: называй реальные риски и что делать, если пойдёт не так. Здесь важна правда, а не только позитив.',
      '',
      'ФОРМАТ ОТВЕТА: строго один JSON-объект, без markdown и пояснений, со схемой:',
      '{',
      '  "архетип": "1 абзац: кто человек как основатель (усиль его тип из поля бизнес.тип) и что это значит для дела",',
      '  "сильные_предприниматель": ["3 сильные стороны как предпринимателя, конкретно по профилю"],',
      '  "слабые_предприниматель": ["2 зоны роста, сформулированы бережно и по делу"],',
      '  "модель_комментарий": "1 абзац про подходящую модель бизнеса (поле бизнес.формат) под этого человека",',
      '  "готовность_комментарий": "1 абзац: честная оценка готовности к запуску по ответам про риск, деньги, ресурсы и сроки",',
      '  "ниши": [ {"название":"название ниши","суть":"1 предложение о сути","свот":{"сильные":"...","слабые":"...","возможности":"...","риски":"...","запасной_ход":"что делать, если пойдёт не так"}} ],',
      '  "план_90": ["5 конкретных шагов на первые 90 дней"]',
      '}',
      'ниши: от 3 до 5, конкретные и реалистичные для России, на пересечении сильных сторон, интересов, выбранной модели и идеи. У каждой ниши обязателен свот с честными рисками и запасным ходом.',
      'Каждый текст — живой и про ЭТОГО человека, без общих слов.'
    ].join('\n');
    const busr = 'Данные профиля предпринимателя (диагностика по 12 методикам + бизнес-блок):\n' +
      JSON.stringify(profile, null, 2) + '\n\nСобери бизнес-разбор по схеме и верни только JSON.';
    return [ { role: 'system', content: bsys }, { role: 'user', content: busr } ];
  }
...
function coerceResult(raw) {
  ...
  // ===== Бизнес-разбор «Своё дело» =====
  if (obj && (Array.isArray(obj['ниши']) || obj['архетип'])) {
    const arr = k => Array.isArray(obj[k]) ? obj[k].map(x => String(x).trim()).filter(Boolean) : [];
    const niches = (Array.isArray(obj['ниши']) ? obj['ниши'] : []).slice(0, 5).map(function (n) {
      n = n || {}; const sw = n['свот'] || {};
      return {
        'название': String(n['название'] || '').trim(),
        'суть': String(n['суть'] || '').trim(),
        'свот': {
          'сильные': String(sw['сильные'] || '').trim(),
          'слабые': String(sw['слабые'] || '').trim(),
          'возможности': String(sw['возможности'] || '').trim(),
          'риски': String(sw['риски'] || '').trim(),
          'запасной_ход': String(sw['запасной_ход'] || '').trim()
        }
      };
    });
    return {
      'архетип': String(obj['архетип'] || '').trim(),
      'сильные_предприниматель': arr('сильные_предприниматель'),
      'слабые_предприниматель': arr('слабые_предприниматель'),
      'модель_комментарий': String(obj['модель_комментарий'] || '').trim(),
      'готовность_комментарий': String(obj['готовность_комментарий'] || '').trim(),
      'ниши': niches,
      'план_90': arr('план_90')
    };
  }
...
function cardPrompt(profile) {
  ...
  const a2  = (profile['архетип_второй'] || '').toString().trim();
  ...
  let p = 'Тёплая вдохновляющая иллюстрация — символический портрет-образ личности';
  if (gender) p += ' (' + gender + ')';
  p += '. Ведущий архетип: ' + a;
  if (a2) p += ' в сочетании с «' + a2 + '»';
  p += '. Раскрой архетип через образ, позу, атмосферу и символические предметы вокруг, чтобы характер читался с первого взгляда';
  if (s)     p += '. Черты: ' + s;
  if (hobby) p += '. Лёгкий акцент — увлечение: ' + hobby;
  if (prof)  p += '. Близко к сфере ' + prof;
  p += '. Светлый стиль, приятные цвета, реалистично, без текста.';
