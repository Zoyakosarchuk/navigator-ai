...
function buildMessages(profile) {
  ...
  const isBiz = /Своё дело|предприним/i.test(track);
  ...
  const sys = [
    ...
    isTeen
      ...
      : (isBiz
        ? '5. Аудитория — взрослый, который хочет своё дело. Тон уважительный и деловой. Сильные стороны, Икигай и навыки связывай с потенциалом собственного бизнеса; в summary подскажи, в каком деле человек раскроется как основатель.'
...
// ---------- Своё дело: ниши со СВОТ (отдельный короткий запрос) ----------
function buildNichesMessages(profile) {
  const sys = [
    'Ты — бизнес-стратег и специалист по подбору ниш для предпринимателей.',
    'По профилю человека подбери ниши для своего дела. Пиши по-русски, на «вы», живо и по делу.',
...
function buildPlanMessages(profile) {
  const niche = profile['ниша'] || 'выбранная ниша';
  const sys = [
    'Ты — наставник предпринимателей. Собери мини-бизнес-план под конкретную нишу для этого человека.',
    'Пиши по-русски, на «вы», конкретно и практично. Опирайся на профиль и выбранную нишу.',
    'ФОРМАТ: строго один JSON-объект, без markdown:',
    '{ "оффер":"что именно продавать и в чём ценность", "клиент":"кому продавать — портрет первого клиента и где его искать", "первые_шаги":["3-4 шага до первого клиента"], "деньги":"сколько примерно нужно на старт и на чём сэкономить, честный ориентир в рублях", "проверка":"как за 1–2 недели проверить спрос малой кровью" }'
  ].join('\n');
...
function cardPrompt(profile) {
  // === Своё дело: образ будущего предпринимателя ===
  if (/Своё дело|предприним/i.test(profile['трек'] || '') || profile['бизнес']) {
    const biz = profile['бизнес'] || {};
    const T = { 'визионер':'визионер, который видит картину целиком', 'эксперт':'эксперт, мастер своего дела', 'системщик':'строитель систем и процессов', 'продавец':'коммуникатор, который умеет продавать' };
    const tp = T[biz['тип']] || 'уверенный предприниматель';
    const g = (profile['пол'] || '').toString().trim();
    let bp = 'Тёплая вдохновляющая иллюстрация — уверенный современный предприниматель';
    if (g) bp += ' (' + g + ')';
    bp += '. Образ основателя: ' + tp + '. Деловая атмосфера успеха, энергия своего дела, взгляд в будущее. Светлый стиль, приятные сине-фиолетовые цвета, реалистично, без текста.';
    return bp.slice(0, 490);
  }
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', async () => {
    ...
    try {
      ...
      // === Своё дело: ниши со СВОТ (отдельный запрос) ===
      if (mode === 'biz_niches') {
        try {
          const raw = await callModel(buildNichesMessages(profile));
      ...
      if (mode === 'biz_plan') {
        try {
          const raw = await callModel(buildPlanMessages(profile));
          let o = raw; if (typeof raw === 'string') { const s = raw.indexOf('{'), e = raw.lastIndexOf('}'); o = JSON.parse(s >= 0 ? raw.slice(s, e + 1) : raw); }
          res.writeHead(200, JSONH);
          return res.end(JSON.stringify({ 'оффер': String(o['оффер'] || '').trim(), 'клиент': String(o['клиент'] || '').trim(), 'первые_шаги': Array.isArray(o['первые_шаги']) ? o['первые_шаги'].map(x => String(x).trim()) : [], 'деньги': String(o['деньги'] || '').trim(), 'проверка': String(o['проверка'] || '').trim() }));
        } catch (e) { console.error('[BIZ] plan ошибка:', e.message); res.writeHead(200, JSONH); return res.end('{}'); }
      }
